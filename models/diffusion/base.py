...
class GaussianDiffusion(nn.Module):
    ...
    def p_loss(
        ...
        # loss_mask = torch.ones_like(x_furture) if loss_mask is None else loss_mask
        
        loss_mask = torch.ones_like(torch.cat([x_0, x_future], dim=1)) if loss_mask is None else loss_mask
        x_t, noise = self.q_step_from_x_0(x_future, steps)
        condition = self.get_network_condition(steps)

        # x_all = torch.cat([x_0, x_t], dim=1)
        prediction = self.model(x_0, x_t, egopose_past, egopose_future, condition)

        target = self.get_target(x_future, steps, noise)
        target_0 = torch.zeros_like(x_0)
        target_all = torch.cat([target_0, target], dim=1)
        # target_all = target_all.half()

        loss = self.criterion(prediction, target_all)  # (B,C,H,W)
        ...
        loss_freq = (torch.fft.rfft2(prediction, dim=(3,4)) - torch.fft.rfft2(target_all, dim=(3,4))).abs().mean()
        loss = loss + 0.008 * loss_freq
        return loss
    ...
    def forward(
        ...
        # shared in continuous/discrete versions
        steps = self.sample_timesteps(x_0.shape[0], x_0.device) # B
        loss = self.p_loss(x_0, x_future, egopose_past, egopose_future, steps, loss_mask)
        return loss
