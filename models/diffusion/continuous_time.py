...
def _log_snr_schedule_cosine(
    ...
    return -2 * _log(torch.tan(t_min + t * (t_max - t_min)))[:, None, None, None, None]
...
class ContinuousTimeGaussianDiffusion(base.GaussianDiffusion):
    ...
    def p_step(
        ...
        alpha_t, sigma_t = _log_snr_to_alpha_sigma(log_snr_t)
        alpha_s, sigma_s = _log_snr_to_alpha_sigma(log_snr_s)
        prediction = self.model(x_past, x_t, egopose_past, egopose_future, log_snr_t[:, 0, 0, 0, 0])
        prediction = prediction[:,5:10]
        if self.objective == "eps":
    ...
    def sample(
        ...
        video_shape = (frames, *self.sampling_shape)
        x = self.randn(batch_size, *video_shape, rng=rng, device=self.device)
        if return_all:
            out = [x]
        ...
        for i in tqdm(range(num_steps), **tqdm_kwargs):
            step_t = steps[:, i]
            step_s = steps[:, i + 1]
            x = self.p_step(x_past, x, egopose_past, egopose_future, step_t, step_s, rng=rng, mode=mode)
            if return_all:
                out.append(x)
