...
class AdaGN(nn.GroupNorm):
    ...
    def forward(self, x: torch.Tensor, emb: torch.Tensor) -> torch.Tensor:
        ...
        # Handle different dimensions of emb
        if emb.ndim == 2:  # (B, dim)
            emb = self.proj(emb)  # (B, out_channels * 2)
            emb = emb.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)  # (B, out_channels * 2, 1, 1, 1)
        elif emb.ndim == 3:  # (B, F, dim)
            B, F, dim = emb.shape
            emb = emb.view(B * F, dim)  # (B*F, dim)
            emb = self.proj(emb)  # (B*F, out_channels * 2)
            emb = emb.view(B, F, -1).permute(0, 2, 1)  # (B, out_channels * 2, F)
            emb = emb.unsqueeze(-1).unsqueeze(-1)  # (B, out_channels * 2, F, 1, 1)
        elif emb.ndim == 5:  # (B, dim, F, H, W)
            B, dim, F, H, W = emb.shape
            # Project along the dim dimension
            emb = emb.permute(0, 2, 3, 4, 1).reshape(-1, dim)  # (B*F*H*W, dim)
            emb = self.proj(emb)  # (B*F*H*W, out_channels * 2)
            emb = emb.view(B, F, H, W, -1).permute(0, 4, 1, 2, 3)  # (B, out_channels * 2, F, H, W)
