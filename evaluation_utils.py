...
# PC_RANGE = [-45.0, -45.0, -2.5, 45.0, 45.0, 2.5]
...
def compute_chamfer_distance_inner(pred_pcd, gt_pcd, device, savename=""):
    ...
    cd_forward, cd_backward, info = chamfer_distance(
...
def spherical_projection(pcd):
    ...
    elevation = np.arctan2(pcd[2], np.sqrt(pcd[0]**2 + pcd[1]**2))
...
def compute_ray_errors(pred_pcd, gt_pcd, origin, device, return_interpolated_pcd=False, savename=""):
    ...
    pred_pcd_interp = origin[None, :] + d_hat[pred_idx.to('cpu')].T * unit_dir
